...
- During grill-me, the agent may answer useful side questions, but captures durable facts as requirements, risks, constraints, open questions, or candidates and then returns to the active readiness question.
- If two consecutive turns do not advance, revise, or explicitly defer the current readiness question, the agent gives a compact checkpoint and asks one next question.
- If the user introduces a different feature or product direction, the agent asks whether to switch focus, record it as a Candidate Change, or return to the current readiness question.
- If the user questions the process, the agent explains why the current confirmation is needed and offers a smaller next step instead of continuing to grill blindly.
...
- For throwaway prototypes, the agent may skip DDD-lite or TDD-first readiness checks only when the shortcut is recorded as a limitation.
...
- Letting early ideation side questions erase the active first-slice readiness question.
