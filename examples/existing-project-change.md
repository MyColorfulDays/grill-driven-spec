...
- If screenshots, browser review, mockups, or a visual companion would help, the agent presents them as optional support after the `Driving` readiness question and a normal text-answerable question are visible.
...
- Review fails if touched OpenSpec artifacts or user-facing readiness text drift into the wrong language.
...
- If multiple active changes exist, the agent lists them with detected readiness state or phase and asks which one to continue.
...
- If the user asks a related side question, the agent answers it, captures durable requirements or risks when needed, and steers back to the current readiness question.
- If two consecutive turns do not advance, revise, or explicitly defer the current readiness question, the agent gives a compact checkpoint with active focus, what needs confirmation before the next stage, what changed, and one next question.
- If the user questions the workflow itself, the agent explains why the current confirmation is needed, adjusts the pace if appropriate, and offers a smaller next step.
- If the user introduces a new feature while another change is active, the agent asks whether to switch focus, record it as a Candidate Change, or return to the current readiness question.
- After archive and spec sync, the agent checks git status and handles commit readiness before calling the workflow fully complete.
...
- If the user manually applied, verified, and archived the change, the agent detects an archived-but-uncommitted state on resume and handles commit readiness with Lore-first policy; normal git is offered only when Lore is unavailable, inappropriate, or explicitly requested.
- When the user expresses bare commit intent after archive, the agent treats that as permission to handle commit readiness with Lore-first policy, not as permission to invent author/committer identity or to use a normal git commit when Lore is available and appropriate.
...
- Letting a helper skill take over readiness checks, artifact routing, or the OpenSpec lifecycle.
- Letting a visual companion obscure the current Driving phase or readiness question during Design Maturity.
...
- Letting side questions accumulate until the active readiness question is no longer visible.
- Ending workflow replies with vague invitations instead of a concrete steering move.
- Treating a workflow/process concern as normal product input instead of pausing to explain or adjust the readiness question.
- Silently switching focus when the user mentions a different feature.
- Recommending the next feature after archive while related archive/spec/docs/implementation changes are still uncommitted and commit readiness is not handled.
...
- Treating missing Lore as a reason to ignore post-archive commit readiness instead of offering normal git commit or user handoff.
